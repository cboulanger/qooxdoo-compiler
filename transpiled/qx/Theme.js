    ...
    statics: {
      ...
      */
      ...
      define: function define(name, config) {
        ...
        config.include = this.__P_9_0(config.include);
        config.patch = this.__P_9_0(config.patch); // Validate incoming data
        ...
        this.__P_9_1(theme, config);

        this.__P_9_2(theme, config); // Store class reference in global class registry
       ...
       */
      __P_9_0: function __P_9_0(objectOrArray) {
        if (!objectOrArray) {
          return [];
        }
       ...
       * @param config {Map} config structure
       */
      __P_9_2: function __P_9_2(theme, config) {
      ...
      */
       ...
       */
      __P_9_3: function __P_9_3(config) {
        for (var i = 0, keys = this.__P_9_4, l = keys.length; i < l; i++) {
          if (config[keys[i]]) {
            return keys[i];
          }
        }
       ...
       */
      __P_9_1: function __P_9_1(theme, config) {
        var type = this.__P_9_3(config); // Use theme key from extended theme if own one is not available
      ...
      __P_9_4: ["colors", "borders", "decorations", "fonts", "icons", "widgets", "appearances", "meta"],
      ...
      patch: function patch(theme, mixinTheme) {
        this.__P_9_8(mixinTheme);

        var type = this.__P_9_3(mixinTheme);

        if (type !== this.__P_9_3(theme)) {
       ...
       */
      __P_9_8: function __P_9_8(mixinTheme) {
...
//# sourceMappingURL=Theme.js.map?dt=1595486312894