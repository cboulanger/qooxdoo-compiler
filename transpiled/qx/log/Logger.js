  ...
  qx.Bootstrap.define("qx.log.Logger", {
    statics: {
      ...
      */
      ...
      setTreshold: function setTreshold(value) {
        this.__P_89_1.setMaxMessages(value);
      },
      ...
      getTreshold: function getTreshold() {
        return this.__P_89_1.getMaxMessages();
      },
      ...
      */
      ...
      register: function register(appender) {
        ...
        var id = this.__P_89_5++;
        var appenderName = appender.appenderName || appender.classname;
        this.__P_89_2[id] = appender;
        this.__P_89_3[appenderName] = appender;
        appender.$$id = id; // Insert previous messages

        var entries = this.__P_89_1.getAllLogEvents();

        for (var i = 0, l = entries.length; i < l; i++) {
          var entry = entries[i];

          var appenders = this.__P_89_6(entry.loggerName, entry.level);
      ...
      unregister: function unregister(appender) {
        ...
        var appenderName = appender.appenderName || appender.classname;
        delete this.__P_89_3[appenderName];
        delete this.__P_89_2[id];
        delete appender.$$id;
      },
      ...
      */
      ...
      debug: function debug(object, message) {
        qx.log.Logger.__P_89_7("debug", arguments);
      },
      ...
      info: function info(object, message) {
        qx.log.Logger.__P_89_7("info", arguments);
      },
      ...
      warn: function warn(object, message) {
        qx.log.Logger.__P_89_7("warn", arguments);
      },
      ...
      error: function error(object, message) {
        qx.log.Logger.__P_89_7("error", arguments);
      },
      ...
      trace: function trace(object, message) {
        if (qx.log.Logger.isLoggerEnabled("trace", object)) {
          ...
          qx.log.Logger.__P_89_7("trace", args);
        }
      },
      ...
      */
      ...
      __P_89_1: new qx.log.appender.RingBuffer(50),
       ...
       */
      __P_89_10: function __P_89_10(object) {
        if (object) {
          if (object.classname) {
      ...
      isLoggerEnabled: function isLoggerEnabled(level, object) {
        var loggerName = this.__P_89_10(object);

        var appenders = this.__P_89_6(loggerName, level);
       ...
       */
      __P_89_7: function __P_89_7(level, args) {
        ...
        for (var i = start, l = args.length; i < l; i++) {
          items.push(this.__P_89_11(args[i], true));
        } // Build entry
        ...
        this.__P_89_1.process(entry); // Send to appenders
       ...
       */
      __P_89_6: function __P_89_6(className, level) {
        var levels = this.__P_89_8; // If no filters, then all appenders apply

        if (!this.__P_89_4.length) {
          // Check the default level
          if (levels[level] < levels[this.__P_89_0]) {
            return [];
          }
        ...
        var appenders = this.__P_89_9[cacheId];
        ...
        for (var i = 0; i < this.__P_89_4.length; i++) {
          var filter = this.__P_89_4[i]; // Filters only apply to certain levels
          ...
          if (!filter.loggerMatch || filter.loggerMatch.test(className)) {
            if (filter.appenderName) {
              appenders[filter.appenderName] = this.__P_89_3[filter.appenderName];
            } else {
              return this.__P_89_9[cacheId] = this.__P_89_3;
            }
          }
        }

        return this.__P_89_9[cacheId] = appenders;
      },
       ...
       */
      __P_89_12: function __P_89_12(value) {
        if (value === undefined) {
          return "undefined";
        } else if (value === null) {
          return "null";
       ...
       * <code>trace</code>.
       */
      __P_89_11: function __P_89_11(value, deep) {
        var type = this.__P_89_12(value);
          ...
          case "array":
            if (deep) {
              ...
              for (var i = 0, l = value.length; i < l; i++) {
                ...
                text.push(this.__P_89_11(value[i], false));
              }
            } else {
              text = "[...(" + value.length + ")]";
            }
          ...
          case "map":
            if (deep) {
              ...
              for (var i = 0, l = sorted.length; i < l; i++) {
                ...
                key = sorted[i];
                temp = this.__P_89_11(value[key], false);
                temp.key = key;
                text.push(temp);
              }
            } else {
    ...
    defer: function defer(statics) {
      ...
      for (var i = 0; i < logs.length; i++) {
        statics.__P_89_7(logs[i][0], logs[i][1]);
      }
...
//# sourceMappingURL=Logger.js.map?dt=1593552891325